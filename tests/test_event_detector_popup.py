...
def test_popup_episode_becomes_one_bonus_group_starting_at_the_popup_onset():
    samples = _ticks(4.0, timer_jumps_s=(1.0,), popup_ranges=((1.0, 1.4),))

    groups = event_detector.detect_popup_kill_groups(samples, session_id=0)

    assert len(groups) == 1
    assert groups[0].timestamp_s == 1.0
...
def test_simultaneous_multi_kill_with_a_single_popup_is_counted_from_the_timer_jump():
    samples = _ticks(4.0, timer_jumps_s=(1.0,), popup_ranges=((1.0, 1.2),))
...
def test_popup_ticks_one_missed_tick_apart_stay_one_episode():
    samples = _ticks(4.0, timer_jumps_s=(1.0,), popup_ranges=((1.0, 1.2), (1.6, 1.8)))
...
def test_popup_ticks_further_apart_than_the_episode_gap_are_separate_episodes():
    far_apart_s = 3.0
    assert far_apart_s - 1.2 > config.POPUP_EPISODE_MAX_GAP_S
    samples = _ticks(8.0, timer_jumps_s=(1.0, far_apart_s), popup_ranges=((1.0, 1.2), (far_apart_s, far_apart_s + 0.2)))

    groups = event_detector.detect_popup_kill_groups(samples, session_id=0)

    assert [g.timestamp_s for g in groups] == [1.0, far_apart_s]


def test_popup_episode_near_a_map_pickup_popup_is_dropped():
    samples = _ticks(4.0, timer_jumps_s=(1.0,), popup_ranges=((1.0, 1.4),), pickup_at=(0.4,))
...
def test_popup_episode_whose_timer_never_jumped_is_dropped():
    samples = _ticks(4.0, popup_ranges=((1.0, 1.4),))
...
def test_combo_rise_beyond_the_popup_bonuses_is_added_as_bullet_kills_on_the_popup_group():
    samples = _ticks(
        4.0, timer_jumps_s=(1.0,), popup_ranges=((1.0, 1.4),), combo_at={0.0: 10, 2.0: 12}
    )

    groups = event_detector.detect_popup_kill_groups(samples, session_id=0)

    assert [(g.timestamp_s, g.group_size) for g in groups] == [(1.0, 2)]


def test_combo_rise_fully_explained_by_the_popup_adds_no_extra_group():
    samples = _ticks(
        4.0, timer_jumps_s=(1.0,), popup_ranges=((1.0, 1.4),), combo_at={0.0: 10, 2.0: 11}
    )

    groups = event_detector.detect_popup_kill_groups(samples, session_id=0)

    assert [(g.timestamp_s, g.group_size) for g in groups] == [(1.0, 1)]


def test_two_popups_inside_one_sparse_combo_pair_stay_two_separate_groups():
    # combo readable only at 0s and 6s (+2): the old adjacent-pair logic
    # merges both kills into one clip; each popup must keep its own.
    samples = _ticks(
        8.0,
        timer_jumps_s=(1.0, 4.0),
        popup_ranges=((1.0, 1.2), (4.0, 4.2)),
        combo_at={0.0: 10, 6.0: 12},
    )

    groups = event_detector.detect_popup_kill_groups(samples, session_id=0)

    assert [(g.timestamp_s, g.group_size) for g in groups] == [(1.0, 1), (4.0, 1)]


def test_combo_rise_without_any_popup_falls_back_to_the_combo_pair_group():
    samples = _ticks(4.0, timer_jumps_s=(1.0,), combo_at={0.0: 10, 2.0: 11})

    groups = event_detector.detect_popup_kill_groups(samples, session_id=0)

    assert [(g.timestamp_s, g.group_size) for g in groups] == [(2.0, 1)]


def test_one_misread_timer_tick_before_the_popup_does_not_inflate_the_bonus_count():
    samples = _ticks(4.0, timer_jumps_s=(2.0,), popup_ranges=((2.0, 2.4),))
    next(s for s in samples if s.timestamp_s == 1.2).timer_value_s -= 20.0
...
def test_one_misread_timer_tick_after_the_popup_does_not_inflate_the_bonus_count():
    samples = _ticks(4.0, timer_jumps_s=(1.0,), popup_ranges=((1.0, 1.4),))
    next(s for s in samples if s.timestamp_s == 2.0).timer_value_s += 20.0
